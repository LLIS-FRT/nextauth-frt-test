...
export const GET = protectedRoute(
    async function GET(req) {
        ...
        const canViewAny = await permissionsChecker([PermissionName.VIEW_ANY_TEAM]);

        if (canViewAny) return new NextResponse(JSON.stringify(teams), { status: 200 });
        else {
    ...
    requiredPermissions: [PermissionName.VIEW_OWN_TEAM]
});
...
export const POST = protectedRoute(
    ...
    requiredPermissions: [PermissionName.CREATE_TEAM]
});
...
export const DELETE = protectedRoute(
    ...
    requiredPermissions: [PermissionName.DELETE_ANY_TEAM]
});
